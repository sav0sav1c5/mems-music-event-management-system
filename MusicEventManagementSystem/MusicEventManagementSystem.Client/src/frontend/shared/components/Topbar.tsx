    ...
    <div className="flex items-center justify-between bg-neutral-900/80 backdrop-blur-sm border border-neutral-800 text-white px-4 py-4 mx-3 mt-4 rounded-xl shadow-lg">
      ...
      <div className="relative w-1/2 mx-1">
        ...
        <input
          type="text"
          placeholder="Search..."
          className="w-full pl-10 pr-2 py-2.5 rounded-2xl bg-neutral-800 border border-neutral-700 text-white placeholder-neutral-500 focus:outline-none focus:ring-1 focus:ring-lime-400 focus:border-transparent transition-all text-base"
        />
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button className="p-3 bg-neutral-800 hover:bg-neutral-700 rounded-xl cursor-pointer transition-all duration-200 border border-neutral-700 hover:border-lime-400/50 group">
            <Bell className="w-5 h-5 text-neutral-400 group-hover:text-lime-400 transition-colors" />
          </button>
          <div className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-lime-400 rounded-full border border-neutral-900"></div>
        </div>

        {/* User Profile */}
        <div className="flex items-center gap-2 bg-neutral-800 hover:bg-neutral-700 px-4 py-2 rounded-xl transition-all duration-200 cursor-pointer border border-neutral-700 hover:border-lime-400/30 group">
          <div className={`w-8 h-8 rounded-full border-2 ${borderColor} bg-neutral-700 flex items-center justify-center font-bold text-xs`}>
            {getDepartmentInitials(user.department)}
          </div>
          <div className="text-xs">
            <div className="font-semibold text-white group-hover:text-lime-400 transition-colors text-sm">
